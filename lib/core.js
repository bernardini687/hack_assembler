...
const resolveLabels = new Transform({
  transform (chunk, _encoding, next) {
    if (chunk.includes('(')) {
      const [label] = chunk.toString().match(/[\w.$-]+/) // TODO: find proper regex for symbol/labels (sys.init, foo1, foo_2, _bar3, __foo.bar4__, ponggame_.run$while_end2)
...
function parseInstructionA (instruction) {
  ...
  if (groups === undefined) {
    groups = i.match(/^(?<symbol>\S+)$/).groups
